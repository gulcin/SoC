/*  
 *  led3.c - advanced led kernel module
 */
...
static ssize_t mode_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count) {
    if (strncmp(buf, "on", count-1)==0) { mode = ON; ledState = true; } // compare with fixed number of chars, count-1 needed to exclude \n
    else if (strncmp(buf, "off", count-1)==0) { mode = OFF; ledState = false; }
